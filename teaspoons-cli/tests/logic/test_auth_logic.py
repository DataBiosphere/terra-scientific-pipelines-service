...
from teaspoons.logic.auth_logic import check_local_token_and_fetch_if_needed, clear_local_token
...
def test_check_local_token_and_fetch_if_needed_already_authenticated(mock_cli_config):
    with patch('teaspoons.logic.auth_logic._load_local_token', return_value='valid_token') as mock_load_token, \
         patch('teaspoons.logic.auth_logic._validate_token', return_value=True) as mock_validate_token, \
...
def test_check_local_token_and_fetch_if_needed_fetch_new_token(mock_cli_config):
    with patch('teaspoons.logic.auth_logic._load_local_token', return_value=None) as mock_load_token, \
         patch('teaspoons.logic.auth_logic.get_access_token_with_browser_open', return_value='new_token') as mock_get_token, \
         patch('teaspoons.logic.auth_logic._save_local_token') as mock_save_token:
...
def test_clear_local_token(mock_cli_config):
    with patch('teaspoons.logic.auth_logic._clear_local_token') as mock_clear_token, \
         patch('builtins.print') as mock_print:
