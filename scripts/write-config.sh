...
Usage: $0 [<target>] [<vaulttoken>] [<outputdir>] [<vaultenv>]"
  ...
  If <vaultenv> is not specified, then use the envvar TEASPOONS_VAULT_ENV
  If TEASPOONS_VAULT_ENV is not specified, then we use docker
EOF
 exit 1
}
...
default_vaultenv=${TEASPOONS_VAULT_ENV:-docker}
