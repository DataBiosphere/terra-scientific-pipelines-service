...
import bio.terra.pipelines.testutils.TestUnitBase;
...
public class TpsBasicControllerTest extends TestUnitBase {
  ...
  //
  //      var inputs = new
  //   ApiTpsPolicyInputs().addInputsItem(groupPolicy).addInputsItem(regionPolicy);
  //
  //      // Create a PAO
  ...
  //
  //      var apiPao = objectMapper.readValue(response.getContentAsString(),
  //   ApiTpsPaoGetResult.class);
  ...
  //      MvcResult result =
  //          mockMvc
  //              .perform(addAuth(addJsonContentType(delete("/api/policy/v1alpha1/pao/" +
  //   objectId))))
  //              .andReturn();
  //      MockHttpServletResponse response = result.getResponse();
  //      HttpStatus status = HttpStatus.valueOf(response.getStatus());
